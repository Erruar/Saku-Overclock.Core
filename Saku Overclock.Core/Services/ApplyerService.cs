    ...
    : IApplyerService
{
    ...
    public async Task ApplyPreset(Preset preset, bool saveInfo = false)
    {
        try
        {
            ...
            var adjline = ParseOverclockPreset(preset);
        ...
        RestoreAppliedSettings() // Запустить все команды после запуска приложения если включен Авто-применять Разгон
    {
        if (settingsService.ReapplyLatestSettingsOnAppLaunch)
        {
            ...
            {
                logger.LogError("ApplyLastSettings failed {ex}", ex);
            }
        }
    }
    ...
    private void ScheduleApplyPreset()
    {
        lock (_applyDebounceLock)
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(presetToApply))
                        {
                            ...
                            {
                                logger.LogError("Invalid custom preset index: {customIndex}", customIndex);
                            }
                            else
                            {
                    ...
                    {
                        logger.LogError("Error applying preset: {ex.Message}", ex.Message);
                    }
                }
                ...
                {
                    logger.LogError("Error in preset scheduling: {ex.Message}", ex.Message);
                }
            }, token);
        }
    }
    ...
    private async Task ApplySettings(bool saveinfo)
    {
        try
        {
            if (settingsService.ReapplyOverclock)
            {
                ...
                try
                {
                    ...
                    if (intervalMs <= 0) throw new ArgumentException("Интервал должен быть больше 0");
                }
                catch
                {
                    logger.LogWarning("Время авто-обновления разгона некорректно и было исправлено на 3000 мс");
                    intervalMs = 3000;
                }
                ...
                lock (_timerLock)
                {
                    ...
                    _timer = new Timer(async void (_) =>
                    {
                        ...
                        {
                            logger.LogError("Overclock settings reapply failed {ex}", ex);
                        }
        ...
        {
            logger.LogError("Overclock settings first apply failed {ex}", ex);
        }
    }
    ...
    private async Task Process(string adjLine, bool saveinfo)
    {
        try
        {
            await Task.Run(() => { sendSmuCommand.Translate(adjLine, saveinfo); });
        }
        catch (Exception ex)
        {
            logger.LogError("Overclock settings apply failed {ex}", ex);
        }
    }

    private string ParseOverclockPreset(Preset preset)
    {
        ...
        if (preset.VrmSettings.VrmPowerSaveVddCurrentLimit.IsEnabled && !isBristol)
            adjline += " --psi0-current=" +
                       ConvertToThousandString(preset.VrmSettings.VrmPowerSaveVddCurrentLimit.Value) +
                       (isBristol
        ...
        if (preset.VrmSettings.VrmPowerSaveSocCurrentLimit.IsEnabled && !isBristol)
            adjline += " --psi0soc-current=" +
                       ConvertToThousandString(preset.VrmSettings.VrmPowerSaveSocCurrentLimit.Value);


        if (preset.VrmSettings.VrmPowerSaveCpuCurrentLimit.IsEnabled)
            adjline += " --psi3cpu_current=" +
                       ConvertToThousandString(preset.VrmSettings.VrmPowerSaveGpuCurrentLimit.Value);

        if (preset.VrmSettings.VrmPowerSaveGpuCurrentLimit.IsEnabled)
            adjline += " --psi3gfx_current=" +
                       ConvertToThousandString(preset.VrmSettings.VrmPowerSaveGpuCurrentLimit.Value);
        ...
        {
            var fp6FeaturesSet = preset.CpuModesSettings.CpuFrequency04Fix.Value != 0
                ? " --disable-feature=0,32"
                : " --enable-feature=0,32";
        ...
        if (preset.CpuSettings.LaptopPowerLimit.IsEnabled)
        {
            ...
            if (isStapmTuneRequired) adjline += " --stapm-limit=" + adjustPower;
        }
        ...
        if (preset.FrequenciesSettings.CpuVoltage.IsEnabled)
            adjline += " --oc-volt=" +
                       (uint)Math.Round((1.55 - preset.FrequenciesSettings.CpuVoltage.Value / 1000) / 0.00625);
        ...
        if (preset.CurveOptimizerOptions.IntegratedGpuCurveOptimizerUndervoltingLevel.IsEnabled)
            adjline += ProcessCoallSettings(
                preset.CurveOptimizerOptions.IntegratedGpuCurveOptimizerUndervoltingLevel.Value, true);
        ...
        // SMU Features
        if (preset.SmuFeaturesSettings.SmuFeaturesOverride) adjline += ProcessSmuFeatures(preset);
    ...
    private static string ProcessDragonRangePerCoreCurveOptimizer(Preset preset)
    {
        ...
        if (CheckCurveOptimizerLenghtAvailability(preset))
            adjline +=
                $" --set-coper={0 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[0] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 1))
            adjline +=
                $" --set-coper={1048576 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[1] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 2))
            adjline +=
                $" --set-coper={2097152 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[2] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 3))
            adjline +=
                $" --set-coper={3145728 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[3] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 4))
            adjline +=
                $" --set-coper={4194304 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[4] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 5))
            adjline +=
                $" --set-coper={5242880 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[5] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 6))
            adjline +=
                $" --set-coper={6291456 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[6] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 7))
            adjline +=
                $" --set-coper={7340032 | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[7] & 0xFFFF)} ";
    ...
    private string ProcessLaptopPerCoreCurveOptimizer(Preset preset, uint cores)
    {
        ...
        if (CheckCurveOptimizerLenghtAvailability(preset, 1))
            adjline +=
                $" --set-coper={(1 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[1] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 2))
            adjline +=
                $" --set-coper={(2 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[2] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 3))
            adjline +=
                $" --set-coper={(3 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[3] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 4))
            adjline +=
                $" --set-coper={(4 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[4] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 5))
            adjline +=
                $" --set-coper={(5 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[5] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 6))
            adjline +=
                $" --set-coper={(6 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[6] & 0xFFFF)} ";

        if (CheckCurveOptimizerLenghtAvailability(preset, 7))
            adjline +=
                $" --set-coper={(7 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[7] & 0xFFFF)} ";

        if (cores > 8)
        {
            if (CheckCurveOptimizerLenghtAvailability(preset, 8))
                adjline +=
                    $" --set-coper={(0x100 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[8] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 9))
                adjline +=
                    $" --set-coper={(0x101 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[9] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 10))
                adjline +=
                    $" --set-coper={(0x102 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[10] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 11))
                adjline +=
                    $" --set-coper={(0x103 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[11] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 12))
                adjline +=
                    $" --set-coper={(0x104 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[12] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 13))
                adjline +=
                    $" --set-coper={(0x105 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[13] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 14))
                adjline +=
                    $" --set-coper={(0x106 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[14] & 0xFFFF)} ";

            if (CheckCurveOptimizerLenghtAvailability(preset, 15))
                adjline +=
                    $" --set-coper={(0x107 << 20) | ((int)preset.CurveOptimizerAdvancedOptions.CurveOptimizerCores.Value[15] & 0xFFFF)} ";
        }
    ...
    private static string ProcessDesktopPerCoreCurveOptimizer(Preset preset)
    {
        return ProcessDragonRangePerCoreCurveOptimizer(preset);
    }

    private void ProcessIrusanovMethod(Preset preset)
    {
        if (cpuService.SmuCoperCommandRsmu == 0) cpuService.SmuCoperCommandRsmu = sendSmuCommand.ReturnCoPer(false);

        if (cpuService.SmuCoperCommandMp1 == 0) cpuService.SmuCoperCommandMp1 = sendSmuCommand.ReturnCoPer();
